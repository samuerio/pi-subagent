...
pi 扩展入口（通过 `package.json` 的 `"pi".extensions` 注册）。`session_start`/`before_agent_start` 钩子全部移除——特化 subagent 作为原生 tool，其发现靠 pi 自身的 tool description，不再需要自定义系统提示词注入。入口职责：用 `pi.registerTool` 注册全部六个 tool——静态实例化 finder/oracle；inline `task` 的 execute 每次调用现读 `subagent.json`、构造临时 `Subagent` 实例（renderCall/renderResult 委托给一个共享默认实例，渲染只依赖 result.details）；`read_session`/`read_session_compaction`/`read_session_entry` 直接转发到 read-session/read-compaction/read-entry 模块的实现函数，均无自定义渲染（harness fallback）。

### `extensions/subagent/read-session.ts`

`read_session` tool：只读查看 pi session JSONL。`session` 参数为双形态（pi CLI `--session <path|id>` 语义）：含 `/`、`\` 或 `.jsonl` 结尾按路径处理（`~` 展开、相对 cwd），否则按 session id 解析（`resolveSessionRef`：`SessionManager.list(cwd)` 先精确后前缀，再 `SessionManager.listAll()` 全局档，first-match 取最近修改）。id 搜索只覆盖默认 sessions 根的一层 `<project>/*.jsonl`，与官方 CLI 相同；更深的 subagent session（`sessions/<tool>/<runId>/`）按设计只走 path（subagent envelope 携带）。envelope 只携带 cwd/entry、message 计数/thinkingLevel/model（session= 与 id= 均不回显：调用方传入的 ref 本身可复用于所有钻取工具，路径只留在 details 供 TUI 调试；entries 与 messages 口径不同——model_change/thinking_level_change 等状态条目投影为零消息，只计入 entries）。经包导出的纯解析管线（`parseSessionEntries` → `migrateSessionEntries` → `buildSessionContext`）返回 resolved transcript（active branch、compaction applied、投影为消息）。渲染时把 compaction/branch entry id 重新对回 `compactionSummary`/`branchSummary` 块头：对 resolved entry 列表与消息列表做 zip（同一 `sessionEntryToContextMessages` 投影，同源同序，精确对齐），任何不一致即放弃全部注解，绝不错标 id。同时导出共享 helper（`expandHome`、`loadSessionEntries`、`formatTranscript`、`resolveSessionRef`）给 read-compaction 复用——本包规模小，不单独抽 session-io 模块。输出不截断：50KB `READ_CAP` 已删（截断内容无获取途径；重读不复合膨胀——toolCall 只渲染 120 字符参数预览）。toolCall part 与 toolResult 消息还都带一个截断的 toolCallId 匹配键 `[call_xxxxxxxx]`（`shortToolCallId`：`call_` 前缀 + 前 8 hex，调用侧与结果侧逐字符一致，使并行 tool call 对应到各自结果；该键取自消息自身，与 entry zip 无关）。entry id 经同一 zip 回标三处钻取句柄（`read_session_entry` 的入口）：toolResult 消息（成功渲染为一行 stub `## toolResult:<name> [call_xxxxxxxx] (id=xxxx, ~size)`，`~size` 为可恢复 text 长度、提示该 stub 是否值得钻取，错误另带 300 字符预览）、含 toolCall part 的 assistant 消息（`[call_xxx]` 匹配键与 `id=` 标在每个 `→` 行的参数 blob 之后，行首内容、行尾元数据）、bashExecution 消息（`!` 命令，`recordBashResult` 以普通 message entry 落盘，块头只带 `## bash (exit=N)`；输出超 300 字符折叠单行并以显式 `⋯ [truncated, full output: read_session_entry id=xxxx]` 标注结尾——`id=` 只出现在截断标注里，短输出已全文渲染无需钻取；显式标注防止被误读为 transcript 截断）。zip 的 message 角色白名单覆盖 user/assistant/toolResult/bashExecution，未知角色才整体放弃注解。
...
**Architecture Invariant:** 必须经 `buildContextEntries(entries, target.parentId)` 重建"触发前一刻的 live context"（上一条 compaction 的已摘要前缀被丢弃、其 summary 被 hoist 到最前），再裁掉 `>= target.firstKeptEntryId` 的 retained tail；裁后前缀 = `[S(n-1)] + [newly-expired raw]`，即该次 `update` 的完整输入。必须 guard `target.parentId`：`buildSessionPath` 对 falsy/未知 leafId 会静默回退到全局最新 entry，取错分支。span 经与 `read_session` 同一 `formatTranscript` 整段渲染，上一个 compaction 落在顶部 `compactionSummary` 块（不再过滤）。中间 summary `S(1)..S(n-1)` 因此可达：每个 `S(k)` 恰好作为 `C_{k+1}` 的 compactionSummary 块出现一次。span 渲染同样传入 `alignAnnotations`，工具结果 stub 在输出里带 `id=`，toolCall/toolResult 另带截断 toolCallId 匹配键 `[call_xxxxxxxx]`。

### `extensions/subagent/read-entry.ts`

`read_session_entry` tool：按 read_session 输出中的 `id=` 钻取单个 entry 的全量内容，按 entry 种类分发——toolResult message：text 全文、`details` JSON（subagent 结果把完整输出保存在 details，此工具是模型侧唯一恢复路径）、非文本部件占位符；bashExecution message（`!` 命令）：命令 + 完整多行输出原文（read_session 的 `## bash` 块只预览 300 字符折叠单行，此处是恢复路径）；assistant message 含 toolCall part：每个调用的完整 pretty-print 参数（对应 120 字符单行预览）。与 read_session_compaction 同构的"发现 → 钻取"配对：stub 头格式是工具间的契约。输出不截断且无 envelope：调用方刚在 transcript 里见过该 entry 的 stub（tool 名/callId/command/exit 都在 stub 行或块头上），回显无增量，身份元数据只留 details 供 TUI 调试。本工具是三个 session 审查工具中唯一有自定义 TUI 渲染的：`renderCall` 在执行前只突出钻取 id（截短 8 hex）+ session 文件名（entry 种类此时不可知，renderCall 不读 JSONL）；`renderResult` 从 `details.kind` 渲染类型摘要行（仅 toolResult 有 `toolResult <tool> [call_xxx]`；bashExecution 与 toolCall 不加头——前者的内容自带 `$ command`，后者的内容首行自带 `## toolCall <name>`，头部只会重复）；toolCall 的 TUI 渲染走 details.preview 紧凑格式（每调用一行 `name {args}`，pretty JSON 全文留给模型），折叠态对齐内置渲染惯例：renderResult 返回自绘 width-aware component（`ToolRenderResultOptions` 不带宽度），按终端宽度截取前 5 个视觉行（长行先折行，预览不超 5 屏行）+ read 渲染器同款 `... (N more lines, to expand)` 提示；取头部而非 bash 的尾部，因为钻取结果的开头（命令行、错误头部）才是识别内容的关键，结果与 call 行之间自补空行、内容逐行染 toolOutput 色（内置渲染器的统一惯例，harness 堆叠 call/result 无内置间距）；展开态显示全文；throw 路径 details 被清空，回退到原始内容文本（错误时染错误色）。

**Architecture Invariant:** 严格只读，不用 `SessionManager.open()`；`session` 经 `resolveSessionRef` 双形态解析；只接受三类钻取目标（toolResult、bashExecution、含 toolCall 的 assistant），其余类型报错指回正确工具（compaction → read_session_compaction；user/custom 消息 read_session 已全文渲染）。
