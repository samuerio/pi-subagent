...
`read_session_entry` tool：按 read_session 输出中的 `id=` 钻取单个 entry 的全量内容，按 entry 种类分发——toolResult message：text 全文、`details` JSON（subagent 结果把完整输出保存在 details，此工具是模型侧唯一恢复路径）、非文本部件占位符；bashExecution message（`!` 命令）：命令 + 完整多行输出原文（read_session 的 `## bash` 块只预览 300 字符折叠单行，此处是恢复路径）；assistant message 含 toolCall part：每个调用的完整 pretty-print 参数（对应 120 字符单行预览）。与 read_session_compaction 同构的"发现 → 钻取"配对：stub 头格式是工具间的契约。输出不截断，envelope 按种类带 `tool=`/`callId=`/`isError=`、`command=`/`exit=` 或 `calls=N`。本工具是三个 session 审查工具中唯一有自定义 TUI 渲染的：`renderCall` 在执行前只突出钻取 id（截短 8 hex）+ session 文件名（entry 种类此时不可知，renderCall 不读 JSONL）；`renderResult` 从 `details.kind` 渲染类型摘要行（`toolResult <tool> [call_xxx]` / `bash (exit=N) $ <cmd>` / `N tool calls`），折叠态只显示头部 + 前 8 行内容，展开态显示全文；throw 路径 details 被清空，回退到原始内容文本（错误时染错误色）。
